    ...
    public class Asm
    {
        ...
        public void RunAsm(int pid)
        {
            ...
            byte[] Asm = this.AsmChangebytes(this.Asmcode);
                ...
                if (pid != 0)
                {
                    addre = VirtualAllocEx(pid, 0, Asm.Length, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
                    WriteProcessMemory(pid, addre, Asm, Asm.Length, 0);
                    threadhwnd = CreateRemoteThread(pid, 0, 0, addre, 0, 0, ref pid);
                    VirtualFreeEx(pid, addre, Asm.Length, MEM_RELEASE);
